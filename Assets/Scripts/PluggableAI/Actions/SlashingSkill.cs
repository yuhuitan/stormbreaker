...
[CreateAssetMenu(menuName ="PluggableAI/Actions/SlashingSkill")]
public class SlashingSkill : Action
{
    ...
    void Attack(StateController controller)
    {
        ...
        if (current_index == 1)
        {
            controller.anim.SetTrigger("slashingleft");
        }
        else if (current_index == 2)
        {
            controller.anim.SetTrigger("slashingright");
        }
        Instantiate(actionPrefab, locations[current_index].position, locations[current_index].rotation);
        Debug.Log("slashing skill");
    }
}
